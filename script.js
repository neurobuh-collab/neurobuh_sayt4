...
/* ---------- 0. Живая таблица на первом экране ---------- */
(function () {
  const cvs = document.querySelector('.hero__grid');
  ...
  /* --- ручки настройки --- */
  const CELL = 46;        // размер ячейки
  const SPAWN = 420;      // как часто вспыхивает одиночная ячейка, мс
  const SWEEP = 7600;     // как часто пересчитывается целая строка, мс
  const PAD = 28;         // отступ от текста, где ячеек нет совсем
  const FADE = 120;       // дистанция, на которой они разгораются до полной силы

  const COLORS = [
    [0, 138, 170, 0.20],   // бирюза
    [43, 182, 217, 0.22],  // циан
    [209, 136, 79, 0.15]   // терракота
  ];

  let W = 0, H = 0, cols = 0, rows = 0;
  let cells = [];
  let running = false, raf = null, last = 0, spawnAcc = 0, sweepAcc = 0;
  ...
  // ячейки не оживают под текстом и плавно разгораются к краям
  ...
  function drawGrid() {
    ...
    ctx.strokeStyle = 'rgba(0,138,170,.07)';
    ctx.lineWidth = 1;
    ctx.beginPath();
  ...
  function light(c, r, color, delay) {
    cells.push({ c: c, r: r, col: color, t: -(delay || 0), life: 1900 + Math.random() * 900 });
  }
  ...
  // строка пересчиталась: ячейки заливаются одна за другой слева направо
  function sweepRow() {
    const r = Math.floor(Math.random() * rows);
    const color = Math.random() < 0.7 ? COLORS[0] : COLORS[2];
    for (let c = 0; c < cols; c++) light(c, r, color, c * 55);
  }
  ...
  function frame(ts) {
    ...
    spawnAcc += dt;
    sweepAcc += dt;
    if (spawnAcc >= SPAWN) { spawnAcc = 0; randomCell(); }
    if (sweepAcc >= SWEEP) { sweepAcc = 0; sweepRow(); }
    ...
    for (let i = cells.length - 1; i >= 0; i--) {
      ...
      const p = cell.t / cell.life;
      if (p >= 1) { cells.splice(i, 1); continue; }
      const x = cell.c * CELL + 3;
      const y = cell.r * CELL + 3;
      const k = falloff(x + CELL / 2, y + CELL / 2);
      if (k <= 0) continue;
      const a = p < 0.22 ? p / 0.22 : (p > 0.55 ? 1 - (p - 0.55) / 0.45 : 1);
      const col = cell.col;
      ctx.fillStyle = 'rgba(' + col[0] + ',' + col[1] + ',' + col[2] + ',' + (col[3] * a * k) + ')';
      box(x, y, CELL - 6, CELL - 6);
    }
  ...
  if (still) return;      // «меньше движения»: остаётся статичная сетка
  ...
  window.addEventListener('resize', () => {
    clearTimeout(t);
    t = setTimeout(() => { resize(); measureSafe(); cells = []; drawGrid(); }, 180);
  });
})();
